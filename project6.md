...
<div style="max-width: 760px; margin: 40px auto; font-family: inherit; line-height: 1.7; padding: 0 15px;">
  ...
  <div style="background: #f7f7f8; border-radius: 8px; padding: 18px 22px; margin-bottom: 30px; font-size: 0.95rem;">
    <div style="display: grid; grid-template-columns: 110px 1fr; row-gap: 6px;">
      ...
      <strong>Team</strong><span>Transmission with one teammate; mechatronic toy with four teammates, split into two sub-teams</span>
  ...
  <p>
    The transmission project is a real scored competition, not just a design exercise: a motor drives your transmission, which powers a bicycle wheel, and a computer records wheel speed over a 180-second run. Score is the wheel's max RPM divided by the time it takes to reach 250 RPM, so the transmission has to balance top speed against how quickly it gets there. Working with one teammate and a $200 budget, we sketched out a function/attribute design tree weighing gear train, belt, and chain-and-sprocket options before settling on a gear train, then chose a 2-stage spur gear reduction over a 3-stage one to keep machining time and part count down.
  </p>
  ...
  <p>
    The biggest challenge across both projects, though, was the same one: acrylic is genuinely hard to machine accurately, and we had designed every hole and dimension ourselves, so there was no one else to blame if a cut came out wrong. With a fixed budget in both cases, a bad cut wasn't just a redo, it was money and stock we didn't have room to lose, which made precision in the shop feel a lot higher-stakes than it does when you're machining to someone else's forgiving tolerances.
  </p>
</div>
